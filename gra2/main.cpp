...
public:
    ...
    Texture(std::string path, int resX, int resY, int moveX, int moveY, sf::RenderWindow& window) {
        if (!texture.loadFromFile(path, sf::IntRect(moveX, moveY, resX, resY))) {
            std::cerr << "Could not load texture " << path << std::endl;
        }
        this->resolution = sf::Vector2f(resX, resY);
        this->size = sf::Vector2f(window.getSize().x / resolution.x, window.getSize().y / resolution.y);
...
Texture z("farmer.png",32,32,0,32,window);
Texture x("farmer speed.png",32,32,0,32,window);
Texture y("tank.png",32,32,0,32,window);
...
public:
    ...
    void update(float t) {
        ...
        if(static_cast<int>(t) % 2 == 0){
            z =   Texture("farmer.png",32,32,0,0,window);
            }

        else{
            z = Texture("farmer.png",32,32,0,32,window);
            }
        if(static_cast<int>(t) % 2 == 0){
            x =   Texture("farmer speed.png",32,32,0,0,window);
            }

        else{
            x = Texture("farmer speed.png",32,32,0,32,window);
            }
            if(static_cast<int>(t) % 2 == 0){
            y =   Texture("tank.png",32,32,0,0,window);
            }

            else{
            y = Texture("tank.png",32,32,0,32,window);
            }
    }
...
public:
    float time;
    Zombie exampleZombies[3];//if y want to add new type of zombie change here and in line 189
    ...
    void Summon(int type,int line){
        zombies[count] = exampleZombies[rand() % 3 ]; // see line 154
...
int main(){
    ...
    Texture sqr("tlo.png",124,124,window);  // texture of a map
    Texture tlo("chmurki.png",124,124,window); // texture of upper part
//here u change background in upper part
    Transform background;
    background.setTexture(tlo,0.2,1/float(n)/2);
    ...
    //  textures of turrets and bullets
    Texture nut("osiol.png",32,32,window);
    Texture pea("krowa.png",32,32,window);
    Texture sun("kura.png",32,32,window);
    Texture sunb("jajko.png",32,32,window);
    Texture bull("mleko.png",32,32,window);
    list<Shop> shop;
    Shop s;
    ...
    p.shootSpeed = 1000;
    Bullet peabullet;
    peabullet.setTexture(bull,0.05);
    peabullet.speed = 0.3;
    peabullet.dmg = 1;
    p.bullet = peabullet;
    p.health = 10;
    ...
    p.health = 10;
    Bullet sunbullet;
    sunbullet.setTexture(sunb,0.035);
...
//if y want to add a new zombie type go to line 154!!!!!!
    Zombie basic;
    basic.speed = 0.03;
    ...
    Zombie speed;
    speed.speed = 0.06;
    speed.setTexture(x,0,0.1);
    speed.health = 2;
    speed.attackSpeed = 150;
    zm.exampleZombies[1] = speed;
    Zombie tank;
    tank.speed = 0.03;
    tank.setTexture(y,0,0.1);
    tank.health = 5;
    tank.attackSpeed = 100;
    zm.exampleZombies[2] = tank;
